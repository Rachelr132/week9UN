...
We can determine rcid from each data set. 
...
We can learn the roll call id for the vote, the country/country code, and how the country voted.
...
```{r}
unvotes |> 
  count(country, vote) |> #counting country and vote
  group_by(country) |> #grouping by country
  mutate(total = sum(n)) |> #adding a column that gets total sum
  mutate(prop_vote = n/total) |> #adding a column of the proportion of votes
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #filters out different countries
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + #creates plot on countries to the proportion of the votes
  geom_col(position = position_stack()) + #stacks each country on top of eachother
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #creates labels
  theme_minimal() +  #color
    scale_fill_viridis_d(end = 0.8) + #changes color
  coord_flip() #flips coordinates
```

> Describe what the graph above demonstrates above UN voting records for the G7 
  
  >The graph above demonstrates that the proportion of the votes were yes. 
...
> What specific countries voted for what
...
> the graph does not show detail of UN country voting so we cannot associate the ranking per country with the graph
...
```{r}
roll_calls |> 
  left_join(issues, by = "rcid") #joins the roll_calls data set to the issues data set 
```

```{r}
roll_calls |> 
  right_join(issues, by = "rcid") #Joins the issues set to the roll_calls data set. 
```

```{r}
roll_calls |> 
  full_join(issues, by = "rcid") #fully joins the data sets 
```

```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") #joins shared columns
```
...
> There is a huge spike, I wonder why there is a huge spike within the time of 1980 to 1990s?
